...
Shader "Custom/Rim/TextureBlend Lit VertexColor Fog" {
	Properties {
		_MainTex ("Main Texture", 2d) = "white" {}
		_BlendTex ("Blend Texture", 2d) = "white" {}
    	_BlendValue ("Blend Value",Range(0.0,1.0)) = 0.0
		//Emission
    	...
    	//Vertex
    	_VertexIntensity ("Vertex Intensity",Range(0.0,3.0)) = 1.0
	}
	SubShader {
		Pass {
			...
			vertexOutput vert(vertexInput v){
				...
				half3 vertexColor = lerp(half3(1.0,1.0,1.0),v.color*_VertexIntensity,clamp(_VertexIntensity,0,1));
				o.col =float4(vertexColor*(emissionColor+saturate(diffuseColor*rimColor)),1.0);
				return o;
			}
			...
			{	
				...
				half4 blendTex = tex2D(_BlendTex,_BlendTex_ST.xy*o.texcoord.xy + _BlendTex_ST.zw);
				half4 tex = lerp(mainTex,blendTex,_BlendValue);
