...
Shader "Custom/Cell/Membrane/Membrane" {
	Properties {
	...
	SubShader {
		Pass{
			...
			vertexOutput vert(vertexInput v){
				...
				half3 diffuseColor=max(0.0,_LightAngle+dot(normalDirection,lightDirection));
			...
			{	
				...
				//Flow
				float4 flowTex = tex2D (_FlowTex, _FlowTex_ST);
				flowTex.xy = flowTex.xy * 2.0 - 1.0;
				flowTex.y *= -1.0;			
				
				float scaledTime = _Time.y * 0.1 + flowTex.z;
				...
				//Normal
				float2 coords = _BumpTex_ST.xy*i.texcoord.xy + _BumpTex_ST.zw;// + ( flowTex.xy * flowA * 0.1 ) ;
				...
				texN = lerp(texN,fixed4(.5,.5,1.0,1.0),.3+.4*flowBlendB);
