...
Shader "Custom/Cell/Nucleus Old" {
	Properties {
	...
	SubShader {
		Pass {
			...
			{	
				...
				half4 waveTex = tex2D (_WaveTex, flowTex.zw * _WaveTex_ST.xy + _Time.y * _WaveTex_ST.zw );
				...
				half4 fireTex1 = tex2D (_FireTex1, o.texcoord.xy * _FireTex1_ST.xy + _Time.y * _FireTex1_ST.zw + ( flowTex.xy * flowA * 0.1 ) );
				half4 fireTex2 = tex2D (_FireTex2, o.texcoord.xy * _FireTex2_ST.xy + _Time.y * _FireTex2_ST.zw + ( flowTex.xy * flowB * 0.1 ) );

				half4 finalFire = lerp( fireTex1, fireTex2, flowBlendB );
				finalFire = lerp( mainTex.x, finalFire, mainTex.w );
				
				half4 Final = lerp(mainTex, finalFire,.8)*mainTex * _Factor * wave;
