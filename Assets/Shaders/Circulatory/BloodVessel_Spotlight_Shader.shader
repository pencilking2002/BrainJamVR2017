...
Shader "Custom/Circulatory/BloodVessel Spotlight" {
	Properties {
//		_Color ("Color", Color) = (1,1,1,1)

		_MainTex ("Emission", 2D) = "white" {}
		_MainTex2 ("Emission 2", 2D) = "white" {}
		...
		_CausticTex1 ("Caustic 1", 2D) = "white" {}
		_CausticTex2 ("Caustic 2", 2D) = "white" {}
	...
	Pass{
		...
		LOD 200
			...
			uniform sampler2D _CausticTex1;
			uniform fixed4 _CausticTex1_ST;
			uniform sampler2D _CausticTex2;
			uniform fixed4 _CausticTex2_ST;
...
//					atten*=2;
			...
			vertexOutput vert(vertexInput v){
				...
				//Save the Y value of the normal in the alpha color
				half3 normalDirection = normalize(mul(float4(v.normal, 0.0), unity_WorldToObject).xyz);
				float causticInfluence = normalDirection.y;
				o.col.a = causticInfluence;
			...
			{	
				...
				fixed4 tex2 = tex2D(_MainTex2,_MainTex2_ST.xy*i.texcoord.xy + _MainTex2_ST.zw*_Time.y);
				fixed4 texN = tex2D(_BumpTex,_BumpTex_ST.xy*i.texcoord.xy + _BumpTex_ST.zw);
				
				//Casutics
				half causticInfluence = i.col.a;
...
//				half3 diffuseReflection = atten  * saturate(dot(normalDirection,lightDirection));
				...
				//final
				half3 texFinal=tex.rgb*tex2.rgb;
				half3 rimFinal = rimLighting;
				half3 emissionFinal = _EmissionColor*_EmissionIntensity;
				
				half3 lightFinal=texFinal*(i.col+emissionFinal+causticInfluence*2);
...
//				half causticInfluence = i.col.a;
				...
				fixed4 texC1 = tex2D(_CausticTex1,_CausticTex1_ST.xy*i.texcoord.xy + _CausticTex1_ST.zw*_Time.y);
				fixed4 texC2 = tex2D(_CausticTex2,_CausticTex2_ST.xy*i.texcoord.xy + _CausticTex2_ST.zw*_Time.y);
				
				
//				half scaledTime = .5*(sin(_Time.y*1)+1);
				half scaledTime = abs(frac(_Time.y*.3)*2-1);
				colorFinal += lerp(texC1,texC2,.1+.8*scaledTime)*.5*clamp(causticInfluence,0,1)*_CausticColor;
//				return float4(scaledTime,scaledTime,scaledTime,1.0);
